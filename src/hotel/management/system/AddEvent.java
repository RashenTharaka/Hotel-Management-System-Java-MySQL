...
public class AddEvent extends JFrame implements ActionListener {
    ...
    public AddEvent() {
        ...
        hourSpinner.setBounds(200, 330, 72, 34);
        add(hourSpinner);

        SpinnerModel minuteModel = new SpinnerNumberModel(0, 0, 59, 1);
        minuteSpinner = new JSpinner(minuteModel);
        minuteSpinner.setBounds(282, 330, 72, 34);
        ...
        amPmComboBox.setBounds(364, 330, 82, 34);
        add(amPmComboBox);
    ...
    public void actionPerformed(ActionEvent ae) {
        if (ae.getSource() == add) {
            ...
            if (!eventnumber.matches("\\d+") || Integer.parseInt(eventnumber) <= 0) {
                UITheme.showMessageDialog(null, "Event Number must be a positive integer!", "Input Error", JOptionPane.ERROR_MESSAGE);
            ...
            if (selectedDate == null) {
                UITheme.showMessageDialog(null, "Please select a date!", "Input Error", JOptionPane.ERROR_MESSAGE);
                ActivityLogger.log(Session.loggedNIC, Session.loggedName, Session.loggedRole,"Attempted to add event ( Event no: " + eventnumber + ")", "Failed");
                return;
            }
            ...
            try {
                ...
                String checkDateQuery = "SELECT COUNT(*) FROM event WHERE date = ?";
                PreparedStatement checkDateStmt = conn.c.prepareStatement(checkDateQuery);
                checkDateStmt.setString(1, date);
                ResultSet rsDate = checkDateStmt.executeQuery();
                rsDate.next();

                if (rsDate.getInt(1) > 0) {
                    UITheme.showMessageDialog(null, "This date is already reserved! Choose another date.", "Date Conflict", JOptionPane.ERROR_MESSAGE);
                ...
                String checkQuery = "SELECT COUNT(*) FROM event WHERE eventnumber = ?";
                PreparedStatement checkStmt = conn.c.prepareStatement(checkQuery);
                checkStmt.setInt(1, Integer.parseInt(eventnumber));
                ResultSet rs = checkStmt.executeQuery();
                rs.next();

                if (rs.getInt(1) > 0) {
                    UITheme.showMessageDialog(null, "Event Number already exists! Choose a different number.", "Duplicate Error", JOptionPane.ERROR_MESSAGE);
                    ActivityLogger.log(Session.loggedNIC, Session.loggedName, Session.loggedRole,"Attempted to add event ( Event no: " + eventnumber + ")", "Failed");
                    return;
                }
                ...
                if (description.isEmpty()) {
                    UITheme.showMessageDialog(null, "All fields must be filled!", "Input Error", JOptionPane.ERROR_MESSAGE);
                    ActivityLogger.log(Session.loggedNIC, Session.loggedName, Session.loggedRole,"Attempted to add event ( Event no: " + eventnumber + ")", "Failed");
                    return;
                }

                String insertQuery = "INSERT INTO event (eventnumber, description, date, time, duration) VALUES (?, ?, ?, ?, ?)";
                PreparedStatement insertStmt = conn.c.prepareStatement(insertQuery);
                insertStmt.setInt(1, Integer.parseInt(eventnumber));
                ...
                UITheme.showMessageDialog(null, "Event Successfully Added");
