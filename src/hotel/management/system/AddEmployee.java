...
public class AddEmployee extends JFrame implements ActionListener{
    ...
    AddEmployee(){
     ...
     public void actionPerformed(ActionEvent ae){
         ...
         if (!isValidNIC(nic)) {
                UITheme.showMessageDialog(null, "Invalid NIC. Enter a valid 10-character old NIC (e.g., 871234567V) or 12-digit new NIC (e.g., 200087123456).");
                ActivityLogger.log(Session.loggedNIC, Session.loggedName, Session.loggedRole,"Attempted to add new employee", "Failed");
                return;
            }
         ...
         try{
             ...
             String checkQuery = "SELECT 1 FROM employee WHERE nic = ?";
             PreparedStatement checkStmt = conn.c.prepareStatement(checkQuery);
             checkStmt.setString(1, nic);
             ResultSet rs = checkStmt.executeQuery();

             if (rs.next()) {
                 UITheme.showMessageDialog(null, "Employee with this NIC already exists!", "Error", JOptionPane.ERROR_MESSAGE);
                 return;
             }

             String query = "INSERT INTO employee (nic, name, age, gender, job, address, phone, email, password, datejoined, salary) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
             PreparedStatement insertStmt = conn.c.prepareStatement(query);
             insertStmt.setString(1, nic);
             insertStmt.setString(2, name);
             insertStmt.setString(3, age);
             insertStmt.setString(4, gender);
             insertStmt.setString(5, job);
             insertStmt.setString(6, address);
             insertStmt.setString(7, phone);
             insertStmt.setString(8, email);
             insertStmt.setString(9, password);
             insertStmt.setString(10, dateJoined);
             insertStmt.setInt(11, salary);
             insertStmt.executeUpdate();

             UITheme.showMessageDialog(null, "Employee Added Successfully");
             ...
             ActivityLogger.log(Session.loggedNIC, Session.loggedName, Session.loggedRole,"Attempted to add new employee: " + name + " (" + nic + ")", "Failed");
             UITheme.showMessageDialog(null, "Error adding employee!", "Error", JOptionPane.ERROR_MESSAGE);
             e.printStackTrace();
         }
