...
public class OrderInventory extends JFrame implements ActionListener {
    ...
    private void updateQuantity() {
        ...
        try {
            Conn c = new Conn();
            PreparedStatement stockStmt = c.c.prepareStatement("SELECT quantity, orders, price FROM inventory WHERE item_name = ?");
            stockStmt.setString(1, selectedItem);
            ...
            if (rs.next()) {
                ...
                if (enteredQty > dbQty) {
                    UITheme.showMessageDialog(null, "Not enough stock");
                    return;
                }
                ...
                PreparedStatement updateStmt = c.c.prepareStatement("UPDATE inventory SET quantity = ?, orders = ? WHERE item_name = ?");
                updateStmt.setString(1, updatedQty);
                updateStmt.setInt(2, newOrderCount);
                updateStmt.setString(3, selectedItem);
                updateStmt.executeUpdate();

                String formattedQty = formatQuantity();
                String insertOrderQuery = "INSERT INTO inventory_orders (item_name, order_quantity, cost) VALUES (?, ?, ?)";
                PreparedStatement insertStmt = c.c.prepareStatement(insertOrderQuery, Statement.RETURN_GENERATED_KEYS);
                insertStmt.setString(1, selectedItem);
                insertStmt.setString(2, formattedQty);
                insertStmt.setDouble(3, Double.parseDouble(totalCostStr));
                insertStmt.executeUpdate();

                ResultSet orderKeys = insertStmt.getGeneratedKeys();
                if (orderKeys.next()) {
                    int orderId = orderKeys.getInt(1); 

                    
                    PreparedStatement orderDateStmt = c.c.prepareStatement("SELECT order_date FROM inventory_orders WHERE order_id = ?");
                    orderDateStmt.setInt(1, orderId);
                    ResultSet orderDateRS = orderDateStmt.executeQuery();
                    if (orderDateRS.next()) {
                ...
                UITheme.showMessageDialog(null, "Inventory order placed.");
                loadInventoryData();
                fillDetails();
            }
            ...
            e.printStackTrace();
            UITheme.showMessageDialog(null, "Error placing order.");
        }
    }
    ...
    public void actionPerformed(ActionEvent ae) {
        ...
        if (ae.getSource() == btnOrder2) {
            ...
            if (result == JFileChooser.APPROVE_OPTION) {
                ...
                try (BufferedReader reader = new BufferedReader(new FileReader(file))) {
                    ...
                    UITheme.showMessageDialog(null, "Inventroy order placed.");
                    ...
                    e.printStackTrace();
                    UITheme.showMessageDialog(null, "Error reading file.");
                }
            }
        }
        ...
        if (ae.getSource() == btnOrder) { 
            ...
            int response = UITheme.showConfirmDialog(null, "Place order?", "Place order", JOptionPane.YES_NO_OPTION);
