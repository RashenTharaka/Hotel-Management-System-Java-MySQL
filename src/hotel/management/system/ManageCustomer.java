...
public class ManageCustomer extends JFrame implements ActionListener {
    ...
    ManageCustomer() {
        ...
        UITheme.centerFeatureWindow(this, 1200, 650);
    ...
    private void loadAvailableRooms() {
        ...
        try {
            ...
            PreparedStatement stmt = c.c.prepareStatement("SELECT roomnumber FROM room WHERE availability = ? AND bed_type = ?");
    ...
    private void loadRoomPrice() {
        ...
        try {
            Conn c = new Conn();
            PreparedStatement stmt = c.c.prepareStatement("SELECT price FROM room WHERE roomnumber = ?");
    ...
    private void addRoomToComboIfMissing(String roomNumber) {
        if (roomNumber == null || roomNumber.trim().isEmpty()) {
            return;
        }
        for (int i = 0; i < selectroomno.getItemCount(); i++) {
            if (roomNumber.equals(selectroomno.getItemAt(i))) {
                return;
            }
        }
        selectroomno.addItem(roomNumber);
    }
    ...
    private void fillDetails() {
        ...
        try {
            Conn c = new Conn();
            PreparedStatement stmt = c.c.prepareStatement("SELECT * FROM customers WHERE name = ?");
            ...
            if (rs.next()) {
                ...
                loadAvailableRooms();
                addRoomToComboIfMissing(currentRoom);
                selectroomno.setSelectedItem(currentRoom);
    ...
    if (ae.getSource() == btnAdd || ae.getSource() == btnUpdate) {
        ...
        if (name.isEmpty() || phone.isEmpty() || nic.isEmpty() || roomNumber == null || price.isEmpty()) {
            UITheme.showMessageDialog(this, "Please fill all fields.");
        ...
        if (!phone.matches("\\d{10}")) {
            UITheme.showMessageDialog(this, "Phone number must be exactly 10 digits.");
            ActivityLogger.log(Session.loggedNIC, Session.loggedName, Session.loggedRole,"Attempted to add or update customer "+name+"( " + nic + ")", "Failed");
            return;
        }
        if (!(nic.matches("\\d{9}[vVxX]") || nic.matches("\\d{12}"))) {
            UITheme.showMessageDialog(this, "Invalid NIC Number.");
            ActivityLogger.log(Session.loggedNIC, Session.loggedName, Session.loggedRole,"Attempted to add or update customer "+name+"( " + nic + ")", "Failed");
            return;
        }
        ...
        try {
            ...
            if (ae.getSource() == btnAdd) {
                ...
                PreparedStatement duplicateStmt = c.c.prepareStatement("SELECT 1 FROM customers WHERE NIC = ?");
                duplicateStmt.setString(1, nic);
                ResultSet rs = duplicateStmt.executeQuery();
                if (rs.next()) {
                    UITheme.showMessageDialog(this, "A customer with this NIC already exists.");
                ...
                String query = "INSERT INTO customers(name, phone, NIC, room_type, room_number, room_price) VALUES (?, ?, ?, ?, ?, ?)";
                PreparedStatement insertStmt = c.c.prepareStatement(query);
                ...
                PreparedStatement roomStmt = c.c.prepareStatement("UPDATE room SET availability = ? WHERE roomnumber = ?");
                roomStmt.setString(1, "Not yet");
                roomStmt.setString(2, roomNumber);
                roomStmt.executeUpdate();
                UITheme.showMessageDialog(null, "Customer Added Successfully");
                ...
                String oldRoom = null;
                PreparedStatement oldRoomStmt = c.c.prepareStatement("SELECT room_number FROM customers WHERE name = ?");
                oldRoomStmt.setString(1, oldName);
                ResultSet oldRoomRs = oldRoomStmt.executeQuery();
                if (oldRoomRs.next()) {
                    oldRoom = oldRoomRs.getString("room_number");
                }

                c.c.setAutoCommit(false);
                String query = "UPDATE customers SET name = ?, phone = ?, NIC = ?, room_type = ?, room_number = ?, room_price = ? WHERE name = ?";
                PreparedStatement updateStmt = c.c.prepareStatement(query);
                updateStmt.setString(1, name);
                updateStmt.setString(2, phone);
                updateStmt.setString(3, nic);
                updateStmt.setString(4, type);
                updateStmt.setString(5, roomNumber);
                updateStmt.setString(6, price);
                updateStmt.setString(7, oldName);
                updateStmt.executeUpdate();

                if (oldRoom != null && !oldRoom.equals(roomNumber)) {
                    PreparedStatement freeOldRoom = c.c.prepareStatement("UPDATE room SET availability = ? WHERE roomnumber = ?");
                    freeOldRoom.setString(1, "Available");
                    freeOldRoom.setString(2, oldRoom);
                    freeOldRoom.executeUpdate();
                }
                PreparedStatement occupyNewRoom = c.c.prepareStatement("UPDATE room SET availability = ? WHERE roomnumber = ?");
                occupyNewRoom.setString(1, "Not yet");
                occupyNewRoom.setString(2, roomNumber);
                occupyNewRoom.executeUpdate();
                c.c.commit();

                UITheme.showMessageDialog(this, "Customer Updated Successfully");
        ...
        try {
            ...
            String selectedName = (String) selectCustomer.getSelectedItem();
                ...
                if (selectedName == null || selectedName.trim().isEmpty()) {
                    UITheme.showMessageDialog(null, "Select customer to delete!");
            ...
            int response = UITheme.showConfirmDialog(this, "Delete this customer?", "Confirm", JOptionPane.YES_NO_OPTION);
            if (response == JOptionPane.YES_OPTION) {
                
                String deletedRoom = null;
                PreparedStatement roomLookupStmt = c.c.prepareStatement("SELECT room_number FROM customers WHERE name = ?");
                roomLookupStmt.setString(1, selectedName);
                ResultSet roomLookupRs = roomLookupStmt.executeQuery();
                if (roomLookupRs.next()) {
                    deletedRoom = roomLookupRs.getString("room_number");
                }

                c.c.setAutoCommit(false);
                PreparedStatement deleteStmt = c.c.prepareStatement("DELETE FROM customers WHERE name = ?");
                deleteStmt.setString(1, selectedName);
                deleteStmt.executeUpdate();

                if (deletedRoom != null) {
                    PreparedStatement freeRoomStmt = c.c.prepareStatement("UPDATE room SET availability = ? WHERE roomnumber = ?");
                    freeRoomStmt.setString(1, "Available");
                    freeRoomStmt.setString(2, deletedRoom);
                    freeRoomStmt.executeUpdate();
                }
                c.c.commit();

                UITheme.showMessageDialog(this, "Customer Deleted Successfully");
        ...
        document.add(revenueTable);
            ...
            if (Desktop.isDesktopSupported()) {
                try {
                    ...
                    ex.printStackTrace();
                    UITheme.showMessageDialog(null, "Cannot open generated PDF automatically.");
                }
            }
            
            UITheme.showMessageDialog(this, "PDF Report generated successfully");
            ActivityLogger.log(Session.loggedNIC, Session.loggedName, Session.loggedRole,"Attempted to genarate customer report", "Success");
        } catch (Exception e) {
            e.printStackTrace();
            UITheme.showMessageDialog(this, "Error generating PDF: " + e.getMessage());
