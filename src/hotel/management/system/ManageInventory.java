...
public class ManageInventory extends JFrame implements ActionListener {
    ...
    private void fillDetails() {
        ...
        try {
            Conn c = new Conn();
            PreparedStatement stmt = c.c.prepareStatement("SELECT * FROM inventory WHERE item_name = ?");
    ...
    public void actionPerformed(ActionEvent ae) {
        ...
        if (ae.getSource() == btnAdd2) {
            ...
            if (result == JFileChooser.APPROVE_OPTION) {
                ...
                try (BufferedReader reader = new BufferedReader(new FileReader(file));
                    ...
                    while ((line = reader.readLine()) != null) {
                        ...
                        if (parts.length == 7) {
                            ...
                            try {
                                ...
                                UITheme.showMessageDialog(null, "Invalid price format: " + parts[3]);
                                continue; 
                            }
                    ...
                    stmt.executeBatch();
                    conn.commit();
                    UITheme.showMessageDialog(null, "Items added");
                    ...
                    ex.printStackTrace();
                    UITheme.showMessageDialog(null, "Error: " + ex.getMessage());
        ...
        if (itemName.isEmpty() || itemName.length() > 20 ||
            ...
            UITheme.showMessageDialog(null, "All fields must be filled correctly");
            return;
        }
        ...
        if (ae.getSource() == btnAdd) {
            try {
                ...
                String checkQuery = "SELECT COUNT(*) FROM inventory WHERE item_name = ?";
                ...
                if (count > 0) {
                    UITheme.showMessageDialog(null, "Item already exists.");
                    return;
                }

                
                String query = "INSERT INTO inventory(item_name, category, quantity, price, supplier_name, supplier_phone) VALUES (?, ?, ?, ?, ?, ?)";
                ...
                insertStmt.setBigDecimal(4, new java.math.BigDecimal(price));
                insertStmt.setString(5, supplierName);
                insertStmt.setString(6, supplierPhone);
                insertStmt.executeUpdate();
                UITheme.showMessageDialog(null, "Item Added Successfully");
            ...
            try {
                Conn c = new Conn();
                String query = "UPDATE inventory SET item_name = ?, quantity = ?, price = ?, category = ?, supplier_name = ?, supplier_phone = ? WHERE item_name = ?";
                PreparedStatement updateStmt = c.c.prepareStatement(query);
                updateStmt.setString(1, itemName);
                updateStmt.setString(2, quantity);
                updateStmt.setBigDecimal(3, new java.math.BigDecimal(price));
                updateStmt.setString(4, category);
                updateStmt.setString(5, supplierName);
                updateStmt.setString(6, supplierPhone);
                updateStmt.setString(7, oldItemName);
                updateStmt.executeUpdate();
                UITheme.showMessageDialog(null, "Item Updated Successfully");
            ...
            int response = UITheme.showConfirmDialog(null, "Delete this item?", "Delete Item", JOptionPane.YES_NO_OPTION);
            if (response == JOptionPane.YES_OPTION) {
                try {
                    Conn c = new Conn();
                    PreparedStatement deleteStmt = c.c.prepareStatement("DELETE FROM inventory WHERE item_name = ?");
                    deleteStmt.setString(1, String.valueOf(selectItemName.getSelectedItem()));
                    deleteStmt.executeUpdate();
                    UITheme.showMessageDialog(null, "Item Deleted Successfully");
    ...
    public void generateReport() {
        try {
            ...
            e.printStackTrace();
            UITheme.showMessageDialog(this, "Error generating report");
