        ...
        if (ae.getSource() == search) {
            searchNIC = searchField.getText().trim();
             ...
             if (searchNIC.isEmpty() || searchNIC.equalsIgnoreCase("Enter NIC to Search")) {
                    UITheme.showMessageDialog(this, "Please enter a NIC to search.", "Validation Error", JOptionPane.ERROR_MESSAGE);
            ...
            try {
                Conn c = new Conn();
                PreparedStatement searchStmt = c.c.prepareStatement(query);
                searchStmt.setString(1, searchNIC);
                ...
                if (rs.next()) {
                    ...
                    UITheme.showMessageDialog(null, "Employee not found.");
            ...
            if(name.equals("")){
                UITheme.showMessageDialog(null, "Name Should Not Be Empty!");
                ActivityLogger.log(Session.loggedNIC, Session.loggedName, Session.loggedRole,"Attempt to update employee", "Failed");
                return;
            }
            
            if(age.equals("")){
             UITheme.showMessageDialog(null, "Age Should Not Be Empty!");
             ActivityLogger.log(Session.loggedNIC, Session.loggedName, Session.loggedRole,"Attempted to add new employee", "Failed");
             return;
            }
            
            if (!rbmale.isSelected() && !rbfemale.isSelected()) {
            UITheme.showMessageDialog(null, "Gender should be select!");
            ActivityLogger.log(Session.loggedNIC, Session.loggedName, Session.loggedRole,"Attempted to add new employee", "Failed");
            return;
            }
            
            if(address.equals("")){
             UITheme.showMessageDialog(null, "Address Should Not Be Empty!");
             ActivityLogger.log(Session.loggedNIC, Session.loggedName, Session.loggedRole,"Attempted to add new employee", "Failed");
             return;
            }
            
            if(phone.equals("")){
                UITheme.showMessageDialog(null, "Phone Number Should Not Be Empty!");
                ActivityLogger.log(Session.loggedNIC, Session.loggedName, Session.loggedRole,"Attempt to update employee", "Failed");
                return;
            }
            
            if (!isValidPhone(phone)) {
                UITheme.showMessageDialog(null, "Invalid phone number. It must be exactly 10 digits.");
                ActivityLogger.log(Session.loggedNIC, Session.loggedName, Session.loggedRole,"Attempted to add new employee", "Failed");
                return;
            }
            
            if (email.isEmpty()) {
                UITheme.showMessageDialog(null, "Email field cannot be empty. Please enter a valid email address.", "Invalid Email", JOptionPane.ERROR_MESSAGE);
                ActivityLogger.log(Session.loggedNIC, Session.loggedName, Session.loggedRole,"Attempt to update employee", "Failed");
                return;
            } else if (!email.contains("@") || !email.contains(".com")) {
                UITheme.showMessageDialog(null, "Please enter a valid email address with '@' and '.com'. For example: user@example.com", "Invalid Email", JOptionPane.ERROR_MESSAGE);
                ActivityLogger.log(Session.loggedNIC, Session.loggedName, Session.loggedRole,"Attempt to update employee", "Failed");
                return;
            }
          ...
          if (job.equals("Admin") || job.equals("Receptionist")) {
            if (!password.equals(confirmPassword)) {
                UITheme.showMessageDialog(null, "Passwords do not match! Please try again.");
            ...
            if (password.isEmpty() || confirmPassword.isEmpty()) {
                UITheme.showMessageDialog(null, "Password and Confirm Password cannot be empty!");
                ActivityLogger.log(Session.loggedNIC, Session.loggedName, Session.loggedRole,"Attempt to update employee: " + name + " (" + searchNIC + ")", "Failed");
                return;
            }

            if (password.length() != 6 || confirmPassword.length() != 6) {
                UITheme.showMessageDialog(null, "Password and Confirm Password must be exactly 6 characters long!");
                ActivityLogger.log(Session.loggedNIC, Session.loggedName, Session.loggedRole,"Attempt to update employee: " + name + " (" + searchNIC + ")", "Failed");
                return;
            }
        } else {
            ...
            try {
                ...
                PreparedStatement updateStmt = c.c.prepareStatement(
                        "UPDATE employee SET name = ?, age = ?, gender = ?, job = ?, address = ?, phone = ?, email = ?, password = ?, salary = ? WHERE nic = ?");
                ...
                updateStmt.setInt(9, salary);
                updateStmt.setString(10, id);
                updateStmt.executeUpdate();
                
                UITheme.showMessageDialog(null, "Employee Updated Successfully");
                ...
                UITheme.showMessageDialog(null, "Employee Updated Failed");
                 e.printStackTrace();
            }
            ...
            String name = tfname.getText();
                ...
                if(searchNIC.isEmpty() || searchNIC.equalsIgnoreCase("Enter NIC to Search") || name.isEmpty()){
                    UITheme.showMessageDialog(null, "Please enter correct NIC of Employee to delete.");
                    ActivityLogger.log(Session.loggedNIC, Session.loggedName, Session.loggedRole,"Attempt to delete employee", "Failed");
                    return;
                }
                
                int response = UITheme.showConfirmDialog(null, "Are you sure you want to delete this employee?", "Delete Employee", JOptionPane.YES_NO_OPTION);
                if (response == JOptionPane.YES_OPTION) {
                    try {
                        Conn c = new Conn();
                        PreparedStatement deleteStmt = c.c.prepareStatement("DELETE FROM employee WHERE nic = ?");
                        deleteStmt.setString(1, id);
                        deleteStmt.executeUpdate();

                        UITheme.showMessageDialog(null, "Employee Deleted Successfully");
                            ...
                            ActivityLogger.log(Session.loggedNIC, Session.loggedName, Session.loggedRole,"Attempt to delete employee", "Failed");
                            UITheme.showMessageDialog(null, "Employee Delete failed");
                            e.printStackTrace();
                    }
                }
         }   
