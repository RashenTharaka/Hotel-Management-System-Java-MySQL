        ...
        if (ae.getSource() == search) {
            try {
                ...
                if (!selectedBed.equals("All")) {
                    query.append(" AND bed_type = ?");
                    params.add(selectedBed);
                }

                if (!selectedAvailability.equals("All")) {
                    query.append(" AND availability = ?");
                    params.add(selectedAvailability);
                }

                Conn conn = new Conn();
                PreparedStatement stmt = conn.c.prepareStatement(query.toString());
...
public void generatePDFReport() {
    ...
    try {
        ...
        e.printStackTrace();
        UITheme.showMessageDialog(null, "Error generating PDF report.");
    }
}
